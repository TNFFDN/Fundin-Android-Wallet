...
import org.fdntrum.imp.AddressStoreImp;
import org.fdntrum.imp.ContextWrapperImp;
import org.fdntrum.imp.WalletConfigurationsImp;
...
import fdntrum.FdntrumPeergroup;
...
*
 * Created by tnf on 1/14/19. on 6/15/17.
...
public class PeergroupTest {
    ...
    @Test
    public void connectFdntrumPeergroupTest() throws IOException, ConnectionFailureException, InterruptedException {
        ...
        FdntrumPeergroup fdntrumPeergroup = new FdntrumPeergroup(networkConf,walletManager,addressStore);
        ...
        //assert fdntrumPeergroup.isRunning():"FdntrumPeergroup is not running..";
    }

    @Test
    public void pushAddressFdntrumPeergroupTest() throws IOException, CantInsertAddressException, ConnectionFailureException, InterruptedException {
        ContextWrapperImp contextWrapperImp = new ContextWrapperImp();
        WalletConfiguration walletConfiguration = new WalletConfigurationsImp();
        NetworkConf networkConf = new NetworkConf("localhost",50001);
        ...
        AddressStore addressStore = new AddressStoreImp();
        FdntrumPeergroup fdntrumPeergroup = new FdntrumPeergroup(networkConf,walletManager,addressStore);
        fdntrumPeergroup.addAddressListener(new AddressListener() {
            @Override
        ...
        fdntrumPeergroup.start();
        while (!fdntrumPeergroup.isRunning()){
            try {
                TimeUnit.SECONDS.sleep(3);
            } catch (InterruptedException e) {
                e.printStackTrace();
        ...
        fdntrumPeergroup.addWatchedAddress(address);
        while (true){
            try {
                TimeUnit.SECONDS.sleep(3);
            } catch (InterruptedException e) {
