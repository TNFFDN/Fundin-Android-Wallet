...
import org.fundinj.core.CoinDefinition;
...
import fdntrum.exceptions.InvalidPeerVersion;
import fdntrum.listeners.AddressListener;
import fdntrum.listeners.PeerDataListener;
import fdntrum.listeners.PeerListener;
...
/**
 * Created by tnf on 1/14/19. on 6/12/17.
 *
 * Class in charge of manage the connection with fdntrum servers.
 *
...
public class FdntrumPeergroup implements PeerListener, PeerDataListener {

    private static final Logger log = LoggerFactory.getLogger(FdntrumPeergroup.class);
    ...
    /** Trusted peer */
    private FdntrumPeer trustedPeer;
    // Currently active peers.
    private final CopyOnWriteArrayList<FdntrumPeer> peers;
    // Currently connecting peers.
    private final CopyOnWriteArrayList<FdntrumPeer> pendingPeers;
    ...
    public FdntrumPeergroup(NetworkConf networkConf, WalletManager walletManager, AddressStore addressStore) throws IOException {
    ...
    public FdntrumPeergroup(NetworkConf networkConf) throws IOException {
        this.peers = new CopyOnWriteArrayList<>();
        this.pendingPeers = new CopyOnWriteArrayList<>();
        this.networkConf = networkConf;
        this.ioManager = new IoManager(1,1);
    ...
    public synchronized void start() throws InterruptedException, ConnectionFailureException {
        try {
            log.info("Starting FdntrumPeergroup");
            isActive = true;
            // todo: first part discovery..
            /*
            * Connect to the trusted node and get servers from it.
            */
            trustedPeer = new FdntrumPeer(networkConf.getTrustedServer(), ioManager,versionMsg);
            ...
            isRunning = false;
            isActive = false;
            log.error("FdntrumPeerGroup start",e);
            throw e;
        }
    }
    ...
    @Override
    public void onConnected(FdntrumPeer fdntrumPeer) {
        try {
            if (fdntrumPeer == trustedPeer) {
                ...
                for (PeerListener peerConnectionListener : peerConnectionListeners) {
                    peerConnectionListener.onConnected(fdntrumPeer);
                }
                ...
                for (InetSocketAddress inetSocketAddress : networkConf.getNetworkServers()) {
                    FdntrumPeerData peerData = new FdntrumPeerData(inetSocketAddress.getHostName(),inetSocketAddress.getPort(),0);
                    FdntrumPeer peer = new FdntrumPeer(peerData,ioManager,versionMsg);
                ...
                log.info("Non trusted peer connected, "+fdntrumPeer.getPeerData());
                pendingPeers.remove(fdntrumPeer);
                peers.add(fdntrumPeer);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    @Override
    public void onDisconnected(FdntrumPeer fdntrumPeer) {

    }

    @Override
    public void onExceptionCaught(FdntrumPeer fdntrumPeer, Exception e) {
    ...
    @Override
    public void onSubscribedAddressChange(FdntrumPeer fdntrumPeer, String address, String status) {
        try {
    ...
    @Override
    public void onListUnpent(FdntrumPeer fdntrumPeer,String address, List<Unspent> unspents) {
    ...
    @Override
    public void onBalanceReceive(FdntrumPeer fdntrumPeer, String address, long confirmed, long unconfirmed) {
        try {
            if (fdntrumPeer == trustedPeer) {
    ...
    @Override
    public void onGetHistory(FdntrumPeer fdntrumPeer, StatusHistory statusHistory) {
        try {
